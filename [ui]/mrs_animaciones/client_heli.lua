...
local mh60t_hash = GetHashKey("mh60t")
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInPolmav() or IsPlayerInEMSHeli() then
...
function IsPlayerInEMSHeli()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, mh60t_hash) or IsVehicleModel(vehicle, maverick_hash)
end
...
function RenderVehicleInfo(vehicle)
	...
	-- print(vehicleSpeedSource, vehicleSpeed)
	local vehicleSpeed = math.ceil(vehicleSpeedSource * 3.6)
	print(vehicleSpeedSource, vehicleSpeed)
	SetTextFont(0)
	SetTextProportional(1)
	SetTextScale(5.0, 0.40)
