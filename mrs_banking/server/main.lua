Citizen.CreateThread(function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bank_accounts` WHERE `account_type` = 'Business'", function(accts)
        buis = #accts
        if accts[1] ~= nil then
...
RegisterServerEvent('mrs_banking:server:modifyBank')
AddEventHandler('mrs_banking:server:modifyBank', function(bank, k, v)
    if banks[tonumber(bank)] then
        banks[tonumber(bank)][k] = v
        TriggerClientEvent('mrs_banking:client:syncBanks', -1, banks)
    end
end)

exports('modifyBank', function(bank, k, v)
    TriggerEvent('mrs_banking:server:modifyBank', bank, k, v)
end)
...
function checkAccountExists(acct, sc)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bank_accounts` WHERE `account_number` = '" .. acct .. "' AND `sort_code` = '" .. sc .. "'", function(exists)
...
RegisterServerEvent('mrs_banking:createNewCard')
AddEventHandler('mrs_banking:createNewCard', function()
    local src = source
...
AddEventHandler('qb-base:itemUsed', function(_src, data)
    if data.item == "moneybag" then
        TriggerClientEvent('mrs_banking:client:usedMoneyBag', _src, data)
    end
end)

RegisterServerEvent('mrs_banking:server:unpackMoneyBag')
AddEventHandler('mrs_banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
...
RegisterServerEvent('mrs_banking:initiateTransfer')
AddEventHandler('mrs_banking:initiateTransfer', function(data)
    --[[
    local _src = source
    ...
    if (checkAccount) then 
        ...
        if receiptName ~= false or receiptName ~= nil then 
            ...
            if userOnline ~= false then
                ...
                if acType == "Current" then
                    ...
                    TriggerClientEvent('mrs_banking:openBankScreen', _src)
                    TriggerClientEvent('mrs_banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('mrs_banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
ESX.RegisterServerCallback('mrs_banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('mrs_banking:createBankCard')
AddEventHandler('mrs_banking:createBankCard', function(pin)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('mrs_banking:openBankScreen', src)
    ESX.ShowNotification('Ha solicitado con éxito una tarjeta de débito.')
    
    TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banking', 'Solicitó con éxito una tarjeta de débito', src)
end)

RegisterServerEvent('mrs_banking:doQuickDeposit')
AddEventHandler('mrs_banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('mrs_banking:openBankScreen', src)
            TriggerClientEvent('mrs_banking:successAlert', src, 'You made a cash deposit of $'..amount..' successfully.')
            TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banking', 'Made a cash deposit of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('mrs_banking:toggleCard')
AddEventHandler('mrs_banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('mrs_banking:doQuickWithdraw')
AddEventHandler('mrs_banking:doQuickWithdraw', function(amount, branch)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then 
            TriggerClientEvent('mrs_banking:openBankScreen', src)
            TriggerClientEvent('mrs_banking:successAlert', src, 'You made a cash withdrawal of $'..amount..' successfully.')
            TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banking', 'Made a cash withdrawal of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('mrs_banking:updatePin')
AddEventHandler('mrs_banking:updatePin', function(pin)
    if pin ~= nil then 
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('mrs_banking:openBankScreen', src)
        TriggerClientEvent('mrs_banking:successAlert', src, 'You have successfully updated your Debit card pin.')
    end
end)

RegisterServerEvent('mrs_banking:savingsDeposit')
AddEventHandler('mrs_banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('mrs_banking:openBankScreen', src)
        TriggerClientEvent('mrs_banking:successAlert', src, 'You made a savings deposit of $'..tostring(amount)..' successfully.')
        TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banking', 'made a savings deposit of $'..tostring(amount)..' successfully..', src)
    end
end)

RegisterServerEvent('mrs_banking:savingsWithdraw')
AddEventHandler('mrs_banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('mrs_banking:openBankScreen', src)
        TriggerClientEvent('mrs_banking:successAlert', src, 'You made a savings withdrawal of $'..tostring(amount)..' successfully.')
        TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banking', 'Made a savings withdrawal of $'..tostring(amount)..' successfully.', src)
    end
end)

RegisterServerEvent('mrs_banking:createSavingsAccount')
AddEventHandler('mrs_banking:createSavingsAccount', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('mrs_banking:openBankScreen', src)
    TriggerClientEvent('mrs_banking:successAlert', src, 'You have successfully opened a savings account.')
